...
public class Time
{
    ...
    public boolean vaildTime(String t)
    {
    ...
    public boolean reasonableLeaveTime(ArrayList<Period> periods, int h, int m, int period, boolean isPm)
    {
        ...
        else
        {
            ...
            this.vaildTime(p.getEnd());
            earliest = this.getHour() * 60 + this.getMinute() - 5;        //5 minutes before previous period ends
            if(p.getPeriod() > 6 && this.getHour() < 12) earliest += 12 * 60;
            Period p2 = periods.get(period - 1);
            this.vaildTime(p2.getStart());
            latest = this.getHour() * 60 + this.getMinute() + 5;          //5 minutes after current period starts (chance of getting in is low to none)
            if(p2.getPeriod() > 6 && this.getHour() < 12) latest += 12 * 60;
